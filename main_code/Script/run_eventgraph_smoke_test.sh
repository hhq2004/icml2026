...
echo "  - 预计时间: 10-12分钟 (包括shot detection + CLIP特征提取)"
echo "========================================================================"
...
# 创建输出目录
mkdir -p ../result/eventgraph_50samples_7b_parallel
...
CUDA_VISIBLE_DEVICES=0 python ../run_inference.py \
    ...
    --num_chunks 4 \
    --chunk_idx 0 \
    --output_dir ../result/eventgraph_50samples_7b_parallel \
    > ../result/eventgraph_50samples_7b_parallel/gpu0.log 2>&1 &
...
CUDA_VISIBLE_DEVICES=1 python ../run_inference.py \
    ...
    --chunk_idx 1 \
    --output_dir ../result/eventgraph_50samples_7b_parallel \
    > ../result/eventgraph_50samples_7b_parallel/gpu1.log 2>&1 &
...
CUDA_VISIBLE_DEVICES=2 python ../run_inference.py \
    ...
    --chunk_idx 2 \
    --output_dir ../result/eventgraph_50samples_7b_parallel \
    > ../result/eventgraph_50samples_7b_parallel/gpu2.log 2>&1 &
...
CUDA_VISIBLE_DEVICES=3 python ../run_inference.py \
    ...
    --chunk_idx 3 \
    --output_dir ../result/eventgraph_50samples_7b_parallel \
    > ../result/eventgraph_50samples_7b_parallel/gpu3.log 2>&1 &

echo "✅ 4个进程已启动，后台运行中..."
echo "📝 日志文件："
echo "  - GPU 0: ../result/eventgraph_50samples_7b_parallel/gpu0.log"
echo "  - GPU 1: ../result/eventgraph_50samples_7b_parallel/gpu1.log"
echo "  - GPU 2: ../result/eventgraph_50samples_7b_parallel/gpu2.log"
echo "  - GPU 3: ../result/eventgraph_50samples_7b_parallel/gpu3.log"
echo ""
echo "⏳ 实时监控进度 (每10秒更新)..."
echo ""
...
while true; do
    ...
    for i in {0..3}; do
        logfile="../result/eventgraph_50samples_7b_parallel/gpu$i.log"
        if [ -f "$logfile" ]; then
            # 提取处理进度 (例如: "Processing:  50%")
...
chunks = []
for i in range(4):
    chunk_file = f'../result/eventgraph_50samples_7b_parallel/VideoMME_EventGraph-LMM_all_Video-LLaVA-7B_chunk{i}.json'
    try:
...
# 保存合并结果
with open('../result/eventgraph_50samples_7b_parallel/VideoMME_EventGraph-LMM_all_Video-LLaVA-7B_merged.json', 'w') as f:
...
echo ""
echo "结果文件: ../result/eventgraph_50samples_7b_parallel/VideoMME_EventGraph-LMM_all_Video-LLaVA-7B_merged.json"
...
for logfile in ../result/eventgraph_50samples_7b_parallel/gpu*.log; do
    if [ -f "$logfile" ]; then
        # 检查真正的Traceback错误
...
if [ "$has_real_error" = true ]; then
    echo "❌ 检测到真实错误！推理失败"
    echo "  查看详细: tail -100 ../result/eventgraph_50samples_7b_parallel/gpu0.log"
